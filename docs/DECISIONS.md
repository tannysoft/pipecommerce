...
## ADR-020: Migrate off Cloudflare Workers → Railway (supersedes ADR-001)

**Date:** 2026-05-11
**Status:** Accepted, supersedes ADR-001

**Context:** หลัง implement MVP เกือบทุก feature, deploy ขึ้น Cloudflare Workers
ผ่าน `@opennextjs/cloudflare` แล้วเจอปัญหา:

- `postgres-js` + Hyperdrive **ค้างเป็นนาทีๆ** ในรันไทม์ Workers — Worker log
  รายงาน "code had hung" และ request timeout ก่อน connection establish
- ลองทุก combination: `fetch_types: false`, `max: 1`, `connect_timeout: 10`,
  ตัด prepared statements — ค้างเหมือนเดิม
- ลองเปลี่ยน DB driver ก็ไม่ practical (Drizzle ส่วนใหญ่ใช้ postgres-js สำหรับ PG)
- middleware ใน Next 16 เปลี่ยนเป็น `proxy.ts` — OpenNext ยังไม่รองรับ Node.js
  middleware ทั้งหมด, ต้อง refactor auth ออกจาก middleware
- `isomorphic-dompurify` ใช้ jsdom — แตกใน Workers, ต้อง regex-based sanitizer
- การ debug ใช้เวลา >1 สัปดาห์ — กระทบ time-to-launch อย่างหนัก

**Options considered:**
- A. ปรับ DB layer ต่อ — ลอง `@neondatabase/serverless` over HTTP, หรือ rewrite
  เป็น Prisma Accelerate. **Reject**: เพิ่ม latency + vendor lock-in + Drizzle
  ecosystem support ไม่ครบ
- B. Vercel — DX ดีสุด แต่แพง, scale cost สูง, vendor lock-in
- C. **Railway** (Node Nixpacks + Postgres plugin) — เหมือน Heroku ยุคใหม่,
  scale ได้, postgres-js ทำงานปกติเพราะเป็น Node server ปกติ
- D. Fly.io — scale ดี แต่ config ซับซ้อนกว่า, MVP launch ช้ากว่า

**Decision:** **Railway** สำหรับ Next.js apps + Railway Postgres แทน Supabase

**Rationale:**
- ออกจาก edge constraint ทั้งหมด → postgres-js, sharp, dompurify ใช้ได้หมด
- Railway Postgres = vanilla Postgres ทุกอย่าง, เปลี่ยน provider ทีหลังได้ง่าย
  (vs Supabase ที่ผูก Auth + Storage + Edge Functions เข้าด้วยกัน)
- Pricing คล้าย Supabase Pro แต่ flexible (pay-per-resource)
- Keep ของดีของ CF: R2 (egress ฟรี) + DNS + Custom Hostnames + `r2-proxy` worker

**Consequences:**
- ❌ ลบ: `wrangler.toml`, `@opennextjs/cloudflare`, `@cloudflare/workers-types`,
  `wrangler` deps, `getCloudflareContext` references, Hyperdrive bindings
- ❌ ลบ: `@pipecommerce/auth` package (Supabase wrapper)
- ❌ ลบ: `@supabase/ssr`, `@supabase/supabase-js`
- ✅ เพิ่ม: `next-auth@5.0.0-beta.29`, `@auth/drizzle-adapter`, `@auth/core`
- ✅ เพิ่ม: Drizzle schema `users` / `accounts` / `sessions` / `verification_tokens`
- ✅ เพิ่ม: `apps/{admin,storefront}/railway.json` + root `nixpacks.toml`
- 🔁 `shop_members.user_id` FK ย้ายจาก `auth.users` (Supabase) → `public.users` (Auth.js)
- 🔁 Custom domain flow เดิม: CF Custom Hostname → Worker route. ใหม่:
  CF Custom Hostname → CNAME → Railway storefront service
- 🔁 Background jobs: CF Queues → **pg-boss** (Postgres-backed) ใน admin service เอง
- 🔁 Cron: CF Cron Triggers → **Railway Cron** → POST `/api/cron/<name>` พร้อม HMAC
- 🔁 Cache (shop-by-domain): CF KV → table + in-memory LRU
- 🔁 Authorization: Supabase RLS ที่อ้าง `auth.uid()` ใช้ไม่ได้แล้ว — ย้ายมา
  check ใน application layer (Server Actions) ผ่าน `requireShop(slug)` helper
- ⚠️ Latency: ไม่มี edge แล้ว — ผู้ใช้ไกล region SG จะช้าลงเล็กน้อย, ชดเชยด้วย
  CF CDN ที่เสิร์ฟ static assets + image จาก R2

**Deployment guide:** see [RAILWAY.md](RAILWAY.md)
