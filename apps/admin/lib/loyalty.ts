 ...
 *
 * Formula:
 *   eligible_amount = subtotal - (totalDiscounts ถ้า earnExcludesDiscounts)
 *   points = floor(eligible_amount / earnRateAmount)
 *
 * เรียกจาก markOrderPaid + Beam webhook
 */
export async function earnLoyaltyForOrder(
  ...
  // Resolve customer (link by email if customerId missing)
  ...
  // Idempotency: ถ้ามี earn entry สำหรับ order นี้แล้ว → skip
  ...
  await db.transaction(async (tx) => {
    ...
    // Upsert customer_loyalty cache (PK = customerId)
    await tx
