import { and, arrayContains, asc, desc, eq, inArray, isNull, min } from '@pipecommerce/db'
  ...
  const productList = await db
    ...
    .where(
      and(
        ...
        arrayContains(products.tags, [tag]),
      ),
    )
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      {productList.length === 0 ? (
        <p className="text-muted-foreground">ไม่พบสินค้าที่ติด tag นี้</p>
      ) : (
